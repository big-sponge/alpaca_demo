...
Alpaca.MainModule.AdminController = {
    ...
    memberListViewAction: function () {
        ...
        view.ready(function () {
            ...
            $('#table-page-search').unbind('click').click(function () {
                var request = {};
                        ...
                        if (data.code == 200 && data.data.list.length > 0) {
                            ...
                            for (var index in role) {
                                ...
                                for (var i in member.role) {
                                    if (member.role[i]['id'] == role[index]['id']) {
                                        is_check = true;
                                        break;
                                    }
                                }
                                if (is_check) {
                                    $('#member-edit [name ="role"]').append('<option selected="selected" value=' + role[index]["id"] + '>' + role[index]["name"] + '</option>');
                                } else {
                                    $('#member-edit [name ="role"]').append('<option value=' + role[index]["id"] + '>' + role[index]["name"] + '</option>');
                                }
                            }
                        }
                    },
                ...
                AlpacaAjax({
                    ...
                    success: function (data) {
                        if (data.code == 200 && data.data.list.length > 0) {
                            ...
                            if (group.permission) {
                                for (var i in group.permission) {
                                    selectIds.push(group.permission[i].id);
                                }
                            }
