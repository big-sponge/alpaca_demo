...
var API = {
    ...
    //个人
    admin_auth_info: '/admin/self/info',            //用户信息
    admin_auth_pwd: '/admin/self/resetPwdByOld',    //重置密码

    //登录、注册权限相关
    admin_auth_login: '/admin/auth/login',         //登录-邮箱密码
    admin_auth_logout: '/admin/auth/logout',       //注销

    //测试-demo
    test_student_list: '/demo/student/list',              //demo 学生列表
    test_student_edit: '/demo/student/edit',              //demo 编辑学生
    test_student_delete: '/demo/student/delete',          //demo 删除学生
    test_score_list: '/demo/score/list',                  //demo 成绩列表
    test_score_edit: '/demo/score/edit',                  //demo 编辑成绩
    test_score_delete: '/demo/score/delete',              //demo 删除成绩
...
AlpacaAjax = function (param) {
    ...
    var success = function (data) {
        ...
        //没有权限
        if (data.code == "fail.permission") {
            Notific({text: data.code});
            return false;
        }
        //系统错误
        if (data.code.indexOf("error") == "1") {
            Notific({text: data.code});
            return false;
        }
    };
...
Alpaca.MainModule = {
    ...
    pageView: function (data) {
        ...
        var userInfo = Alpaca.MainModule.getUserInfo();
        if (empty(userInfo)) {
            return Alpaca.MainModule.emptyView();
        }
...
var tablePageDisplay = function (data) {
    ...
    data.totalNum = Math.ceil(data.count / data.size);
    ...
    //设置页面
    $(place + ' [name ="table-page-info" ]').text("显示 " + data.from + " 到 " + data.to + " 共 " + data.count + " 行");
    ...
    $(place + ' .table-pagination').twbsPagination({
        ...
        onPageClick: function (event, page) {
            data.request._callback(data.request._param, {
                pageNum: page,
                pageSize: $(place + ' [name ="table-page-size" ]').val()
            });
        }
    });
